This directory contains the Grub4DOS boot records that are used by SkyImager
...
* Note that, for convenience reasons, the first 512 bytes from this grldr.mbr are
  *not* the ones that SkyImager processes when writing the actual MBR (first 512 bytes).
  Instead, the byte array from src/ms-sys/inc/mbr_grub.h (whose content is identical)
  is what SkyImager uses. If you have modified this file, and the MBR section is altered,
